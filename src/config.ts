...
export interface SerpApiConfig {
  ...
  platforms: string[];
  /** Geographic origin of the search (e.g. "Bengaluru, Karnataka, India"). Undefined/empty = Google picks based on country. */
  location?: string;
  /** SerpApi/Google Jobs chip filter (e.g. "date_posted:week", "employment_type:fulltime"). Empty = no chip filter. */
  chips?: string;
}
...
function buildConfig(raw: ConfigFile): AppConfig {
  return {
    cycle: {
      ...
      staleness_days: raw.cycle?.staleness_days ?? DEFAULT_STALENESS_DAYS,
      dedup_strategy: raw.cycle?.dedup_strategy ?? DEFAULT_DEDUP_STRATEGY,
      max_jobs_per_hour: raw.cycle?.max_jobs_per_hour ?? DEFAULT_MAX_JOBS_PER_HOUR,
    ...
    daemon: {
      ...
      monthly_search_cap: raw.daemon?.monthly_search_cap ?? DEFAULT_MONTHLY_SEARCH_CAP,
      search_warn_threshold: raw.daemon?.search_warn_threshold ?? DEFAULT_SEARCH_WARN_THRESHOLD,
    },
    serpapi: {
      ...
      platforms:
        raw.serpapi?.platforms === undefined
          ...
          : Array.isArray(raw.serpapi.platforms)
            ...
            : [...DEFAULT_SERPAPI_PLATFORMS],
      location: normalizeOptionalString(raw.serpapi?.location),
      // Explicit "" disables the default chip filter; omitted → use default.
      chips:
        raw.serpapi?.chips === undefined
          ? DEFAULT_SERPAPI_CHIPS || undefined
          : normalizeOptionalString(raw.serpapi.chips),
    },
    openai: {
    ...
    extraction: {
      resume_max_chars: raw.extraction?.resume_max_chars ?? DEFAULT_RESUME_MAX_CHARS,
      description_max_chars: raw.extraction?.description_max_chars ?? DEFAULT_DESCRIPTION_MAX_CHARS,
      chat_max_tokens: raw.extraction?.chat_max_tokens ?? DEFAULT_CHAT_MAX_TOKENS,
      analyze_max_tokens: raw.extraction?.analyze_max_tokens ?? DEFAULT_ANALYZE_MAX_TOKENS,
      score_max_tokens: raw.extraction?.score_max_tokens ?? DEFAULT_SCORE_MAX_TOKENS,
      profile_extraction_max_tokens:
        raw.extraction?.profile_extraction_max_tokens ?? DEFAULT_PROFILE_EXTRACTION_MAX_TOKENS,
    },
    scoring: {
...
export function validateConfig(cfg: AppConfig): void {
  ...
  if (cfg.cycle.staleness_days < 0) errs.push('cycle.staleness_days must be >= 0');
  if (cfg.cycle.max_jobs_per_hour < 1) errs.push('cycle.max_jobs_per_hour must be >= 1');
  ...
  if (cfg.daemon.monthly_search_cap < 1) errs.push('daemon.monthly_search_cap must be >= 1');
  if (cfg.daemon.search_warn_threshold < 0) {
    errs.push('daemon.search_warn_threshold must be >= 0');
  }
  if (cfg.daemon.search_warn_threshold > cfg.daemon.monthly_search_cap) {
    errs.push('daemon.search_warn_threshold cannot exceed daemon.monthly_search_cap');
  }
  ...
  // extraction
  for (const [k, v] of Object.entries(cfg.extraction)) {
    if (typeof v !== 'number' || v < 1) errs.push(`extraction.${k} must be a positive number`);
  }
  ...
  // budget warning (not an error — just surface it)
  const cyclesPerMonth = Math.floor((30 * 86400) / cfg.daemon.poll_interval_seconds);
  const searchesPerMonth = cyclesPerMonth * cfg.cycle.queries.length * cfg.cycle.max_pages_per_query;
  if (searchesPerMonth > cfg.daemon.monthly_search_cap) {
    logger.warn('config budget warning: projected searches exceed monthly cap', {
      queries: cfg.cycle.queries.length,
      max_pages_per_query: cfg.cycle.max_pages_per_query,
      cycles_per_month: cyclesPerMonth,
      projected_searches: searchesPerMonth,
      monthly_search_cap: cfg.daemon.monthly_search_cap,
      hint: 'raise poll_interval_seconds or trim queries',
    });
  }
