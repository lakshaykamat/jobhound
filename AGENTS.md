...
Local HTTP server with a Tailwind dashboard at `http://localhost:8787`. Boots paused; the UI exposes Start / Stop / Run-once controls and streams live cycle + job events via SSE. A cycle: discover jobs (SerpApi Google Jobs) -> analyze -> score against resume profile -> upsert to the local jobs store. Cost and usage tracking is local. Full spec in `docs/prd.md`.

## Codex Agent Instructions

- Treat this file as the root repo guidance for Codex and other coding agents.
- Read relevant code before editing; prefer existing project patterns over new abstractions.
- Keep changes narrowly scoped to the requested behavior.
- Do not revert or overwrite unrelated user changes in the working tree.
- Use `rg` / `rg --files` for repo searches when available.
- Use `apply_patch` for manual file edits.
- Start or run only the checks needed for the change, and report any checks that could not be run.
- If frontend behavior changes, verify the UI in a browser when a local server is available.
...
- **Dedup before LLM.** Known `job_id` skips Analyze/Score, does not mutate the existing row, and emits a `skipped-known` event.
...
From PRD section 11: cross-source dedup rule, hosting target, scorer calibration. Flag these instead of silently resolving them.

Resolved: **write-once row semantics**. The server writes a row when it first sees and scores a posting. After that it never touches the row: no `last_seen` bumps, no staleness sweep, no status mutation. Known `job_id`s are read for dedup only; the user owns the row state from then on.

Resolved: **server starts paused**. `src/cli/server.ts` boots the HTTP listener but does not auto-start the cycle loop. The UI POSTs `/api/start`, `/api/stop`, `/api/run-once`. SSE on `/api/events` broadcasts `state`, `cycle:start`, `cycle:finish`, `cycle:error`, `job`, and `usage` events. The UI lives in `web/` as a single Tailwind-CDN HTML page plus `app.js`; no build step.
