...
The rest (`score_threshold`, `serpapi.*`, `openai.model`, `scoring.*`, `server.poll_interval_seconds`, …) has sane defaults — leave them unless you have a reason. The server re-reads `config.json` at the start of every cycle, so edits take effect on the next run without restart.
...
```bash
docker compose run --rm jobhound node dist/cli/server.js --once
```
...
A stopped server loses nothing — the Sheet has every job, and `.data/` (mounted as a volume) has every meter and event.
...
`.data/` is mounted from the host (`./.data` ↔ `/app/.data`) so contents survive `docker compose down/build/up`. **If you delete `.data/`, the server will reset month-to-date usage to zero and ignore the cap until it accumulates again.**
...
| Server won't stop on Ctrl-C | Use `docker compose down`. The container forwards SIGTERM; the server aborts its sleep and exits within a few seconds. |
...
├── Dockerfile                # Multistage build → node dist/cli/server.js
└── src/
    ├── adapters/             # External I/O: llm, serpapi, sheets, tracker
    ├── cli/                  # Entrypoints: server, verify-sheet
